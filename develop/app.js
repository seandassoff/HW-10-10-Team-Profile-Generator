...
// Validation
const required = (input) => (input ? true : "Required");
// QUESTIONS
const adminChoices = [
  {
    type: "list",
    ...
    name: "adminchoice",
    choices: ["Add an employee to the team?", "Create the team HTML page?"],
    // validate: required,
  },
];
...
let start = async function adminStart() {
  await Inquirer.prompt(adminQuestions).then(async function (userData) {
    let managerInfo = {
      name: userData.name,
...
let next = async function adminNext() {
  await Inquirer.prompt(adminChoices).then(async function (answers) {
...
let input = async function init() {
  await Inquirer.prompt(questions).then(async function (userData) {
    let userInfo = {
      name: userData.name,
      id: JSON.parse(userData.id),
      email: userData.email,
...
async function buildManager() {
  await Inquirer.prompt(managerQuestion).then(async function (userData) {
    let managerAns = {
    ...
    .then(async function () {
      ...
      axios
        ...
        .then(async function (response) {
          ...
          //   console.log("buildEngineer -> engineer", engineer);
...
async function buildIntern() {
  await Inquirer.prompt(internQuestion).then(async function (userData) {
    let internInfo = {
